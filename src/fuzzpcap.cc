...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size)
    ...
    int argc = 3;
    char* argv[4] = {(char*)"bro", (char*)"-r", (char*)"/tmp/ramfuzz/fuzz.pcap", NULL};
	bro_argc = argc;
    ...
    mkdir("/tmp/ramfuzz", 0700);
    int fuzzfd = open("/tmp/ramfuzz/fuzz.pcap", O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
    if (fuzzfd == -1) {
        printf("open failed\n");
        return 0;
    }
    if (ftruncate(fuzzfd, Size) == -1) {
        printf("ftruncate failed\n");
        return 0;
    }
    if (write (fuzzfd, Data, Size) != Size) {
        printf("write failed\n");
        return 0;
    }
    close(fuzzfd);
